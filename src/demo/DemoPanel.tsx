...
import { DASHBOARD_PERSONAS } from './personas';
 ...
 * Mounted by src/components/DevTools.tsx, which only reaches this module in a
 * dev build with ?demo=1, so nothing here can land in a production bundle.
 */
export const DemoPanel: React.FC = () => {
  ...
  const [activeDashboardPersonaId, setActiveDashboardPersonaId] = useState<string | null>(null);
  ...
  const handleSwitchPersona = useCallback(
    (index: number) => {
      ...
      setActiveDashboardPersonaId(null);
  ...
  const handleSwitchDashboardPersona = useCallback(
    (id: string) => {
      const persona = DASHBOARD_PERSONAS.find((p) => p.id === id);
      if (!persona) return;
      setActiveDashboardPersonaId(id);
      setUser(persona.user);
      setStoredUser(persona.user);
      setSubscriptionLocal(persona.subscription);
      saveSubscription(persona.subscription);
      ...
      showToast(`پرسونای داشبورد «${persona.label}» بارگذاری شد.`, 'info');
    },
    [setUser, setSubscriptionLocal, showToast]
  );
    ...
    <>
      ...
      >
        <div className="space-y-5 text-ink">
          ...
          {/* Dashboard personas: exercise the /org role guards */}
          <div className="pt-3 border-t border-sunken">
            <p id="demo-f4" className="block text-meta font-bold text-ink/80 mb-2">
              پرسونای داشبورد سازمانی:
            </p>
            <div className="space-y-2" role="group" aria-labelledby="demo-f4">
              {DASHBOARD_PERSONAS.map((p) => (
                <button
                  key={p.id}
                  onClick={() => handleSwitchDashboardPersona(p.id)}
                  className={`w-full min-h-[48px] p-3 rounded-tile text-right text-meta transition-all border cursor-pointer ${
                    activeDashboardPersonaId === p.id
                      ? 'bg-domain-1-tint border-primary font-bold text-ink shadow-xs'
                      : 'bg-surface border-sunken hover:bg-canvas text-ink/80'
                  }`}
                >
                  <span className="font-bold block">{p.label}</span>
                  <span className="text-meta text-ink/70 mt-1 leading-relaxed font-normal block">
                    {p.desc}
                  </span>
                </button>
              ))}
            ...
            <a
              href="/org/overview"
              className="mt-2 min-h-[44px] px-3 py-2 rounded-tile bg-surface hover:bg-canvas border border-sunken text-center text-meta font-bold text-primary flex items-center justify-center"
            >
              رفتن به داشبورد سازمان (/org)
            </a>
          </div>
